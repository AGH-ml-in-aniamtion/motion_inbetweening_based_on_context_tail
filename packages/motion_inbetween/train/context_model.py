...
def eval_on_dataset(
    ...
    if fixed_pred_window_start is not None:
        assert fixed_pred_window_start >= beg_contex_len, "fixed_pred_window_start must be greater or same as beg_context"
        window_len = window_len - beg_contex_len + fixed_pred_window_start
        target_idx = fixed_pred_window_start + trans_len
        seq_slice = slice(fixed_pred_window_start, target_idx)
    ...
    # attention mask
    if fixed_pred_window_start is None:
        atten_mask = get_attention_mask(
            window_len, beg_contex_len, target_idx, device, past_context_len)
    else:
        atten_mask = get_attention_mask(
            window_len, fixed_pred_window_start, target_idx, device, past_context_len) 
    ...
    #TODO: fix below. Can i cut beginning in positions so i do not have to care about fixed pos. 
    ...
    for i, data in enumerate(data_loader, 0):
        ...
        if fixed_pred_window_start is not None:
            positions, rotations = data_utils.to_start_centered_data(
                positions, rotations, fixed_pred_window_start)
        else:
            positions, rotations = data_utils.to_start_centered_data(
        ...
        pos_new, rot_new = evaluate(
            ...
            indices, mean, std, atten_mask, past_context_len, post_process, unknown_beginning_len=fixed_pred_window_start-beg_contex_len) 
        ...
        if fixed_pred_window_start is not None:
         ...
         npss_batch_loss, npss_batch_weights) = \
            benchmark.get_rmi_style_batch_loss(
                ...
                fixed_pred_window_start, target_idx, mean_rmi, std_rmi
            ) 
        else:
            (gpos_batch_loss, gquat_batch_loss,
            npss_batch_loss, npss_batch_weights) = \
        ...
        if save_results:
            dirname = "./beg{}_past{}_fixed{}".format(
                beg_contex_len, past_context_len, fixed_pred_window_start)
...
def evaluate(model, positions, rotations, seq_slice, indices,
    ...
    with torch.no_grad():
        ...
        data_mask = get_data_mask(
            ...
            target_idx, device, dtype, past_context, midway_targets=midway_targets, unknown_beginning_len=unknown_beginning_len)
